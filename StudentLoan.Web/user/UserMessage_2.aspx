<%@ Page Title="" Language="C#" MasterPageFile="~/user/UserMain.Master" AutoEventWireup="true" CodeBehind="UserMessage_2.aspx.cs" Inherits="StudentLoan.Web.user.UserMessage_2" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <title>个人信息 - 通知</title>
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <div class="content">

        <div class="tabs">

            <ul>
                <li><a href="UserMessage.aspx">我最近的动态</a></li>
                <li class="active"><a href="javascript:;">通知</a></li>
            </ul>

        </div>

        <div class="cont">

            <div class="item">

                <div class=" border-radius p10 mb10">
                    <div class="media">
                        <a class="pull-left" href="#">
                            <img class="media-object" data-src="holder.js/64x64" alt="64x64"
                                src="data:image/png;base64,"
                                style="width: 64px; height: 64px;">
                        </a>

                        <div class="media-body">
                            <h5 class="media-heading c-red">材料驳回</h5>

                            <p>共一封</p>

                            <p>2014-07-14</p>

                            <p>
                                我喜欢网购，也喜欢理财。我是理智型购物者，愿意价比三家；我属于平衡型投资者不会盲目跟风。进入网贷圈3年，接触了不少平台，我善于抓住稳定和收益的平衡点。小马bank的包商银行背景吸引了我；
                            </p>

                            <p>
                                <button type="button" class="btn btn-primary">打开</button>
                                <button type="button" class="btn btn-danger">删除</button>
                            </p>
                        </div>
                    </div>
                </div>

                <div class=" border-radius p10 mb10">
                    <div class="media">
                        <a class="pull-left" href="#">
                            <img class="media-object" data-src="holder.js/64x64" alt="64x64"
                                src="data:image/png;base64,"
                                style="width: 64px; height: 64px;">
                        </a>

                        <div class="media-body">
                            <h5 class="media-heading c-green">材料通过</h5>

                            <p>共一封</p>

                            <p>2014-07-14</p>

                            <p>
                                我喜欢网购，也喜欢理财。我是理智型购物者，愿意价比三家；我属于平衡型投资者不会盲目跟风。进入网贷圈3年，接触了不少平台，我善于抓住稳定和收益的平衡点。小马bank的包商银行背景吸引了我；
                            </p>

                            <p>
                                <button type="button" class="btn btn-success">关闭</button>
                                <button type="button" class="btn btn-danger">删除</button>
                            </p>
                        </div>
                    </div>
                </div>

                <div class=" border-radius p10 mb10">
                    <div class="media">
                        <a class="pull-left" href="#">
                            <img class="media-object" data-src="holder.js/64x64" alt="64x64"
                                src="data:image/png;base64,"
                                style="width: 64px; height: 64px;">
                        </a>

                        <div class="media-body">
                            <h5 class="media-heading c-red">材料驳回</h5>

                            <p>共一封</p>

                            <p>2014-07-14</p>

                            <p>
                                我喜欢网购，也喜欢理财。我是理智型购物者，愿意价比三家；我属于平衡型投资者不会盲目跟风。进入网贷圈3年，接触了不少平台，我善于抓住稳定和收益的平衡点。小马bank的包商银行背景吸引了我；
                            </p>

                            <p>
                                <button type="button" class="btn btn-primary">打开</button>
                                <button type="button" class="btn btn-danger">删除</button>
                            </p>
                        </div>
                    </div>
                </div>

                <div class=" border-radius p10 mb10">
                    <div class="media">
                        <a class="pull-left" href="#">
                            <img class="media-object" data-src="holder.js/64x64" alt="64x64"
                                src="data:image/png;base64,"
                                style="width: 64px; height: 64px;">
                        </a>

                        <div class="media-body">
                            <h5 class="media-heading c-green">材料通过</h5>

                            <p>共一封</p>

                            <p>2014-07-14</p>

                            <p>
                                我喜欢网购，也喜欢理财。我是理智型购物者，愿意价比三家；我属于平衡型投资者不会盲目跟风。进入网贷圈3年，接触了不少平台，我善于抓住稳定和收益的平衡点。小马bank的包商银行背景吸引了我；
                            </p>

                            <p>
                                <button type="button" class="btn btn-success">关闭</button>
                                <button type="button" class="btn btn-danger">删除</button>
                            </p>
                        </div>
                    </div>
                </div>

            </div>

            <div class="clear mt20"></div>

            <div class="pagination pagination-centered">
                <ul>
                    <li><a href="#">«</a></li>
                    <li><a href="#">1</a></li>
                    <li><a href="#">2</a></li>
                    <li><a href="#">3</a></li>
                    <li><a href="#">4</a></li>
                    <li><a href="#">5</a></li>
                    <li><a href="#">»</a></li>
                </ul>
            </div>

        </div>

    </div>
</asp:Content>
